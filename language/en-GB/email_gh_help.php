<div>
    ...
    <ul>
        ...
        <li>The File Attachments box allows you to set one or more files to be attached to the email. You can enter a input name in curly brackets e.g. {input_file_1}, a file path for an existing file, or a comma separated list of entries. File paths can use the <tt>{#path#}</tt> placeholder to shorten the path. This will be replaced by the path to the Joomla! root folder - see the help under the box for the current value.</li>
    ...
    <ul>
        ...
        <li>The one special case is the From Email box which will only accept a valid email address. Using a dynamic From Email address is the most frequent cause of failed emails. Use a dynamic Reply To Name and Reply To Email to make it easy to reply to the form submitter. Note: If you are an experienced user and you need to use the dynamic From Email box you can enable the curly brackets by adding ##OK## to the box.</li>
